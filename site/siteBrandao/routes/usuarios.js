...
router.post('/entrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select * from usuario where login='${email}' and senha='${senha}'`);
  }).then(consulta => {
...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select count(*) as contagem from usuario where login = '${email}'`);
  }).then(consulta => {
    ...
    res.status(500).send(erro);
	  ...
	  if (cadastro_valido) {		  
		...
		banco.sql.query(`insert into usuario (nome, login, senha) values ('${nome}','${email}','${senha}')`).then(function() {
